...
class CadConfig:
    ...
    min_area: float = field(default_factory=lambda: float(os.getenv("CAD_MIN_AREA", "0.5")))
...
def get_layer_whitelist() -> list:
    cfg = get_cad_config()
    return [l.strip() for l in cfg.layer_whitelist.split(",") if l.strip()]


def get_layer_blacklist() -> list:
    cfg = get_cad_config()
    return [l.strip() for l in cfg.layer_blacklist.split(",") if l.strip()]
