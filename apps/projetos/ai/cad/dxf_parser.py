...
from apps.projetos.ai.cad.ir import GeometryIR, Segment, Arc, Circle, Spline, Ellipse
...
def _dispatch(entity, ir: GeometryIR, doc) -> None:
    ...
    ir.track_entity(layer, dxftype)
    ir.current_layer = layer

    # TEXT/MTEXT/DIMENSION carregam dados semânticos — nunca filtrar por layer
    if dxftype in ("TEXT", "MTEXT"):
        _parse_text(entity, ir)
        return

    # Filtrar layers de anotação/decoração para geometria
    # (não aplica a TEXT/MTEXT/DIMENSION que já foram tratados acima)
    if dxftype not in ("DIMENSION",):
        from apps.projetos.ai.cad.config import is_layer_allowed
        if not is_layer_allowed(layer):
            return
        # Filtragem adicional: layers de anotação conhecidas
        layer_upper = layer.upper().strip()
        _ANNOTATION_LAYERS = frozenset({
            "COTAS", "DEFPOINTS", "TEXTOS", "CARIMBO", "HACHURA",
            "DIM", "ANOTAÇÃO", "ANOTACAO", "LEGENDA", "GRID",
            "EIXOS", "EIXO", "VIEWPORT",
            "TÍTULO", "TITULO", "MARGEM", "SELO", "QUADRO", "NORTE",
        })
        _ANNOTATION_SUBSTRINGS = ("COTA", "DIM", "ANNOT", "HATCH", "LEGENDA", "CARIMBO", "GRID")
        if layer_upper in _ANNOTATION_LAYERS:
            return
        for substr in _ANNOTATION_SUBSTRINGS:
    ...
    if dxftype == "LINE":
        ...
        ir.add_error(f"Entidade nao reconhecida ignorada: {dxftype}")
...
# ─── SOLID, 3DFACE, LEADER, MLINE, DIMENSION ────────────────
    ...
    if dp2 is not None and dp is not None:
        if dp2.distance(dp) > 1e-10:
            ...
            ir.add_dimension(
                ...
                measurement=dp2.distance(dp),
                defpoint=dp, defpoint2=dp2, defpoint3=dp3,
            )
...
def _parse_insert(entity, ir: GeometryIR, doc,
    ...
    if _depth > MAX_BLOCK_NESTING:
        ir.add_error(f"INSERT aninhado >{MAX_BLOCK_NESTING} niveis ignorado (possivel referencia circular)")
        return
    ...
    if block_name in doc.blocks:
        ...
        if block is None:
            ir.add_error(f"INSERT ignorado: bloco '{block_name}' nao encontrado (XREF nao resolvido)")
            return
    ...
    ir.add_block(block_name, entity.dxf.layer, world_insert, world_rotation, world_x_scale, world_y_scale)
...
def _parse_block_entity(block_entity, ir: GeometryIR, doc,
    ...
    ir.current_layer = getattr(block_entity.dxf, "layer", "0")
    if ir.current_layer == "0":
        ir.current_layer = block_name  # fallback to block name or we could pass down the INSERT layer.
        
    # Let's pass the INSERT layer down, but `layer` is not passed to _parse_block_entity.
    # Actually, we can just use the INSERT layer if block_entity layer is 0.
    # Wait, the INSERT entity layer was passed as `entity.dxf.layer` in `_parse_insert`.
    # Let's just use `block_entity.dxf.layer` for now.
    ir.current_layer = getattr(block_entity.dxf, "layer", "0")
...
def _bulge_to_arc(p1: Vec2, p2: Vec2, bulge: float) -> Optional[Arc]:
    """
    Converte bulge DXF para arco usando a fórmula canônica.

    bulge = tan(included_angle / 4)
    Sinal: positivo = sentido anti-horário (CCW), negativo = horário (CW)
    """
    if abs(bulge) < 1e-10:
        return None
    ...
    # Fórmula canônica DXF
    alpha = 2.0 * math.atan(abs(bulge))  # metade do ângulo incluído
    radius = chord / (2.0 * math.sin(alpha))

    # Distância do ponto médio da corda ao centro
    d = math.sqrt(max(radius * radius - (chord / 2.0) ** 2, 0.0))

    mid = (p1 + p2) / 2
    # Vetor perpendicular à corda (normalizado)
